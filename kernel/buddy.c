...
//Set bit at position index in alloc
...
// Clear bit at position index in alloc
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (bit_isset(bd_sizes[k].alloc, buddy/2) == 1) {  // is buddy allocated?
...
bd_initfree_pair(int k, int bi, int flag) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, bi/2) == 1) {
    // one of the pair is free
    free = BLK_SIZE(k);
    if(flag == 0) {
      int i = buddy < bi ? bi : buddy;
      lst_push(&bd_sizes[k].free, addr(k, i));
    }else{
       int i = buddy < bi ? buddy : bi;
      lst_push(&bd_sizes[k].free, addr(k, i));
    }
